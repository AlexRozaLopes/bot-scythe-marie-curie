...
pub async fn create_role_imunidade(ctx: &Context, _framework: FrameworkContext<'_, Data, Error>, data_about_bot: &Ready) -> Result<(), Error> {
    ...
    for g in guilds {
        ...
        let role_imu: Vec<Role> = roles.iter().filter(|r| r.name == role.name).map(|i| i.clone()).collect();
